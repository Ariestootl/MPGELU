...
\section{Proposed Activation Function}
\subsection{Formulation}

\noindent The standard Gaussian Error Linear Unit (GELU) relies on the fixed variance of the standard normal distribution $\mathcal{N}(0, 1)$, which limits the network's ability to adapt to varying data distributions across layers. To address this, we propose a parameterized extension by introducing a learnable scaling parameter $\lambda \geq 1$.\\

\noindent To strictly enforce this lower bound during unconstrained gradient-based optimization, we introduce a parameter $s \in \mathbb{R}$ and map it through the Softplus function:
\begin{equation*}
\lambda = 1 + \ln(1 + e^s)
\end{equation*}
Using this constrained parameter, we formulate our proposed activation function by scaling the input within the cumulative distribution function:
\begin{equation*}
...
\subsection{Non-Monotonicity and Critical Points}
\noindent Unlike standard ReLU, which is strictly monotonic, our proposed activation function is non-monotonic, exhibiting a slight negative dip before rising asymptotically. To locate this global minimum, we set the first derivative to zero:
\begin{equation*}
f'(x) = \Phi(\lambda x) + \lambda x \phi(\lambda x) = 0
\end{equation*}

\noindent Solving this transcendental equation yields a unique negative critical point $x^* < 0$. This localized "negative pocket" allows the network to occasionally assign small negative values to inputs below a certain threshold, providing a soft, adaptive gating mechanism that helps regularize weights and prevent over-saturation during training.
